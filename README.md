...
- PPT 模板管理：[ppt-template-api.md](./ppt-template-api.md)

部署说明（生产 / Docker）：[DEPLOY.md](./DEPLOY.md)（test155 见 [当前构建方式](./DEPLOY.md#当前构建方式test155)：本机构建镜像 + 离线传 `vanna-api:latest`）
...
编辑 `.env`，填入 MySQL 连接与时间字段（业务表常用 `stat_time`）：
...
MYSQL_TABLES=vd_daily
DATE_COLUMN=stat_time
```

训练 Vanna 向量库（自动从 MySQL 读取表结构）：
...
| 文件 | 作用 |
|------|------|
| `app.py` | FastAPI 服务入口，定义 HTTP 路由（`chart_ai_query`、`chart_sql_query`、`database_tables`、PPT 模板等）；通过 `services.config` 加载 `.env` 与日志配置 |
| `start.sh` | 一键启动脚本：激活 `venv` 后执行 `python app.py` |
| `Dockerfile` | Docker 镜像构建（`python:3.10-slim`，支持构建代理） |
| `docker-compose.yml` | Docker Compose 编排（兼容 docker-compose v1） |
...
| `ppt-template-api.md` | PPT 模板管理接口文档 |
| `DEPLOY.md` | 部署文档（本地、生产 systemd、Docker + 代理） |

### `services/` 业务代码

| 文件 | 作用 |
|------|------|
| `config.py` | **环境配置**。项目根目录 `.env` 在此统一加载；各模块通过 `get_env()` 读取变量 |
...
| `dual_period_sql.py` | **基准合并 SQL**。将同一份基础 SQL 分别套上 `dataPeriod` / `baselinePeriod`，用 `LEFT JOIN` 拼成一条查询，产出「基准」列；列名仅预览一次（数据期与基准期结构相同） |
| `period.py` | **日期处理**。校验 `YYYYMMDD` 格式；`apply_date_period_to_sql` 向 WHERE 注入日期（字段由 `DATE_COLUMN` 配置）；`normalize_base_sql_for_period_merge` 清理 AI 写死的日期 CASE |
| `baseline.py` | **基准列后处理**。约定列名为「基准」且固定最后一列；过滤「基准总行驶里程」等冗余列；调整 `tableMatrix` 列顺序（基准值在 `dual_period_sql` JOIN 中计算） |
| `table_matrix.py` | **结果格式转换**。将 `DataFrame` 转为前端所需的 `tableMatrix`（二维字符串数组），并按 `chartType` 裁剪列数 |
| `database.py` | **MySQL 访问**。长连接与断线重连、执行 SELECT、`SHOW CREATE TABLE` 拉取表结构供训练使用 |
| `ppt_template.py` | **PPT 模板存储**。独立 SQLite 持久化模板 JSON，提供列表/保存/详情/删除 |
...
| 文件 | 作用 |
|------|------|
| `train.py` | 训练 Vanna 向量库：导入表结构 DDL、字段说明、示例问答；切换业务库后需重新执行并**重启服务** |
| `pack.sh` | 打包项目为 `dist/*.tar.gz`，上传线上部署 |
| `deploy.sh` | 线上解压后执行：`docker-compose build && up -d` |
| `export-base-image.sh` | 导出 Docker 基础镜像 `python:3.10-slim` 为离线包 |
| `load-base-image.sh` | 在目标机器加载离线基础镜像 |
| `list_tables.py` | 命令行查看「MySQL 已连接表」与「Vanna 已训练表」 |
...
| `data/ppt_templates.db` | PPT 模板库（运行时生成，不提交 Git） |
...
├── ppt-template-api.md         # PPT 模板接口文档
├── services/
│   ├── config.py               # .env 加载与 get_env()
│   ├── chart_query.py          # 接口编排
...
│   ├── ppt_template.py         # PPT 模板 CRUD
│   └── query_log.py            # 查询日志
├── scripts/
│   ├── train.py                # 训练 Vanna 向量库
│   ├── list_tables.py          # 查看已连接/已训练表
│   ├── pack.sh                 # 打包部署
│   ├── deploy.sh               # 线上 Docker 部署
│   ├── export-base-image.sh    # 导出离线基础镜像
│   └── load-base-image.sh      # 加载离线基础镜像
└── data/
    ├── ppt_templates.db        # PPT 模板库（运行时生成）
    └── chroma/                 # 向量库
```

## 切换业务数据库

修改 `.env` 中的数据库配置后，重新执行训练脚本并重启服务（Vanna 实例有内存缓存）：

```bash
python scripts/train.py
# 重启 app.py 或 docker-compose restart
```
...
| `HOST` | 服务监听地址 | `0.0.0.0` |
| `PORT` | 服务端口 | `8080` |
| `DB_TYPE` | 数据库类型（当前仅支持 `mysql`） | `mysql` |
| `MYSQL_HOST` | MySQL 主机 | - |
| `MYSQL_PORT` | MySQL 端口 | `3306` |
| `MYSQL_USER` | MySQL 用户名 | - |
| `MYSQL_PASSWORD` | MySQL 密码 | - |
| `MYSQL_DATABASE` | MySQL 库名 | - |
| `MYSQL_TABLES` | 训练/导入指定表（逗号分隔） | 全部表 |
| `DATE_COLUMN` | 时间过滤字段（AI 查询 `dataPeriod` / `baselinePeriod` 注入 WHERE） | `stat_time` |
